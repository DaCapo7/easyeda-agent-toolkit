...
这一篇讲整条链路里最枯燥也最容易卡死的一段：**手里有一份网表，怎么让它变成嘉立创工程里画好的原理图、
再变成 PCB 上摆好的器件**。全程不用手画一根线。
...
**为什么这一步值得单独花时间——一个真实例子。**

有个两脚的螺丝端子，符号上两个引脚只隔 10 个单位。生成脚本给每个引脚拉了一段 +20 长度的水平短线，
结果这两段线**跨过了对方的引脚**，把本该分开的两个网络（电源输入和地）粘成了一个。

原理图 ERC 报 0，什么都看不出来。是逐脚比对把它抓出来的。

修法：删掉那两条坏线（`sch_PrimitiveWire.delete`），改成**竖直错位**的短线
（`create([x, y, x, y±60], net)`）重画，再导出重新比对确认归零。

普通电阻电容不会中招，因为它们的引脚间距有 100 左右，20 的短线够不着。
**引脚密集的器件都要提防这个。**
